...
Version 0.51.0 adds glossary help tooltips across the redesigned Statistics tabs so technical concepts like percentile, rarity, population, and fairness are explained inline without changing the underlying analysis or filtering behavior. Stored sessions, rolls, expression syntax, and public broadcast/API formats are unchanged.

Version 0.52.2 seeds four centrally configured, user-editable Players statistics, migrates existing Players preferences once, accepts validated arbitrary `dN` Roll die text, and removes Category and date filters only from Players custom statistics. Ledger and Outcomes filters, stored sessions, rolls, expression syntax, and public broadcast/API wire formats are unchanged.
