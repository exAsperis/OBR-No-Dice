...
Version 0.27.0 adds extensible Query shortcuts: PbtA, PbtA 10+, Nat 20, Nat 1, Snake Eyes, and Boxcars. Query now has independent final Result and individual Roll filters, including an optional die type, so natural d20 faces are matched even when the final result has modifiers. Results and Dice still select the displayed outcome distribution. This is a compatible feature release; the roll ledger and broadcast API formats are unchanged.
