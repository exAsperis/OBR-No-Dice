...
export function summary(rolls:StoredRoll[]){
  ...
  return {rolls:rolls.length,dice:rolls.reduce((n,r)=>n+r.resolution.dice.length,0),players:players.size,expressions:expressions.size,first:ordered[0]?.timestamp,last:ordered.at(-1)?.timestamp,duration,perHour:duration>0?rolls.length*3_600_000/duration:null,longestGap,busiestTen:busiest(600_000),most:ranking(p),least:ranking(p,true),expression:ranking(e),die:ranking(new Map([...diceTypes].map(([label,count])=>[label,{label,count}])))};
}
