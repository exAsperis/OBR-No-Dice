...
const cache=new Map<string,Distribution|null>();
export function theoretical(keyName:string):Distribution|null {
  ...
  try { const separator=keyName.indexOf(':');const dialect=keyName.slice(0,separator) as 'nodice'|'roll20';const ast=parse(expressionLabel(keyName),dialect);const chart=distribution(ast);if(chart.exact&&chart.entries.length&&chart.entries.every(e=>finite(e.value)))result=chart; } catch { /* Unsupported expression. */ }
  cache.set(keyName,result);return result;
}
export const clearTheoreticalCache=()=>cache.clear();
/** Discrete midpoint percentile: P(X < x) + P(X = x)/2. Extremes use inclusive tails. */
export function percentile(roll:StoredRoll){
  if(!finite(roll.finalResult))return null;
  const chart=theoretical(roll.normalizedExpression);if(!chart)return null;
  let lower=0,equal=0,upper=0;
  for(const item of chart.entries){const value=item.value as number;if(value<roll.finalResult)lower+=item.probability;else if(value>roll.finalResult)upper+=item.probability;else equal+=item.probability;}
  if(equal<1e-12)return null;
  return {score:lower+equal/2,tail:Math.min(lower+equal,upper+equal),mean:chart.mean,range:chart.range};
}
...
export function expressionStats(rolls:StoredRoll[]){const groups=new Map<string,StoredRoll[]>();for(const r of rolls){const list=groups.get(r.normalizedExpression)??[];list.push(r);groups.set(r.normalizedExpression,list);}return [...groups].map(([id,list])=>{const values=list.map(r=>r.finalResult).filter(finite);const chart=theoretical(id);return {id,label:expressionLabel(id),rolls:list.length,rollers:new Set(list.map(r=>r.rollerId)).size,expected:chart?.mean??null,range:chart?.range,observed:values.length?values.reduce((a,b)=>a+b,0)/values.length:null,min:values.length?Math.min(...values):null,max:values.length?Math.max(...values):null,distribution:outcomeDistribution(list,'results'),chart,list};}).sort((a,b)=>b.rolls-a.rolls);}
export function timeline(rolls:StoredRoll[],bucket?:number){if(!rolls.length)return {bucket:60_000,points:[] as {time:number;count:number}[]};const times=rolls.map(r=>r.timestamp);const first=Math.min(...times),last=Math.max(...times);const natural=last-first<3_600_000?60_000:last-first<21_600_000?300_000:600_000;const size=bucket??Math.max(natural,Math.ceil((last-first)/120/natural)*natural);const start=Math.floor(first/size)*size;const counts=new Map<number,number>();for(const time of times){const at=Math.floor(time/size)*size;counts.set(at,(counts.get(at)??0)+1);}const points=[];for(let at=start;at<=last;at+=size)points.push({time:at,count:counts.get(at)??0});return {bucket:size,points};}
