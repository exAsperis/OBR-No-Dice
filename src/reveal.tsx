...
import { canStartAutoDismiss, canUseRevealActions, isStoredResult } from './rollLifecycle';
...
function Reveal() {
  ...
  useEffect(() => {
    ...
    OBR.onReady(async () => {
      ...
      local.onmessage = (event: MessageEvent<unknown>) => {
        ...
        if(event.data.type==='stored'){setStoredRequestId(event.data.requestId);return;}
        if(event.data.type==='storage-error'){setRerolling(false);setRerollError(event.data.message);return;}
        ...
        setRerolling(false);
        setRerollError('');
        setStoredRequestId(null);
        setResult(event.data.result);
      };
      local.postMessage({ type: 'ready', ...identity.current } satisfies LocalMessage);
      local.postMessage({type:'presentation-state-request',...identity.current} satisfies LocalMessage);
    });
  ...
  useEffect(() => {
    if (!autoDismiss || !result || !canStartAutoDismiss(result.requestId,storedRequestId,rerolling,visibleCount>=lines.length,momentFrameDoneRequestId===result.requestId)) { setDismissTiming(null); return; }
    ...
    return () => window.clearTimeout(timer);
  }, [autoDismiss, dismissSeconds, result, storedRequestId, rerolling, visibleCount, lines.length,momentFrameDoneRequestId]);

  function dismiss() {
    if (!result||!isStoredResult(result.requestId,storedRequestId))return;
  ...
  function reroll() {
    if (!result || !identity.current || presentationBusy || !canUseRevealActions(result.requestId,storedRequestId,rerolling)) return;
    setRerolling(true);
    setRerollError('');
