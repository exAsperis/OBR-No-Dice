...
export function LedgerTab({rolls,session,roomId='',viewerId=''}:{rolls:StoredRoll[];session?:DiceSession;roomId?:string;viewerId?:string}){
  ...
  return <section className="sequence-tab" aria-label="Ledger">
    ...
    <div className="sequence-controls">
      <label><HelpTerm help="expressionRegex">Expression regex</HelpTerm><input type="text" value={pattern} onChange={event=>setPattern(event.target.value)} placeholder="^2d6(?:[+-]\d+)?$" maxLength={200} spellCheck={false}/></label>
      <label><HelpTerm help="result">Result</HelpTerm><select value={resultComparison} onChange={event=>setResultComparison(event.target.value as ResultComparison)}>{operators.map(operator=><option key={operator} value={operator}>{operatorLabel(operator)}</option>)}</select></label>
      {resultComparison!=='any'&&<label><HelpTerm help="result">Result value</HelpTerm><input type="number" value={resultValue} onChange={event=>setResultValue(event.target.value)}/></label>}
      <label><HelpTerm help="rollFilter">Roll</HelpTerm><select value={rollComparison} onChange={event=>setRollComparison(event.target.value as ResultComparison)}>{operators.map(operator=><option key={operator} value={operator}>{operatorLabel(operator)}</option>)}</select></label>
      {rollComparison!=='any'&&<><label><HelpTerm help="rollFilter">Roll value</HelpTerm><input type="number" value={rollValue} onChange={event=>setRollValue(event.target.value)}/></label><label><HelpTerm help="rollDie">Roll die</HelpTerm><select value={rollDie} onChange={event=>setRollDie(event.target.value)}><option value="">Any die</option>{allDieTypes.map(type=><option key={type}>{type}</option>)}{rollDie&&!allDieTypes.includes(rollDie)&&<option>{rollDie}</option>}</select></label></>}
      <label><HelpTerm help="interpretationCategory">Category</HelpTerm><select value={category} onChange={event=>setCategory(event.target.value)}><option value="">Any</option>{categories.map(item=><option key={item}>{item}</option>)}</select></label>
