...
export interface DiceSession { id:string; name:string; startedAt:number; endedAt?:number }
...
export function readSharedSession(metadata:Record<string,unknown>):DiceSession|undefined {const raw=metadata[SESSION_KEY] as Partial<DiceSession>|undefined;return raw&&typeof raw.id==='string'&&typeof raw.name==='string'&&typeof raw.startedAt==='number'?{id:raw.id,name:raw.name,startedAt:raw.startedAt}:undefined;}
...
export async function appendRoll(room:string,player:string,result:RollResult,shared?:DiceSession):Promise<void>{if(result.error)return;const localScope=scope(room,player);const database=await db();if(await request(database.transaction('rolls').objectStore('rolls').get(`${localScope}/${result.requestId}`)))return;const session=await ensureSession(room,player,shared);await insert(room,player,result,session.id);await prune(room,player);}
