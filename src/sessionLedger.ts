...
export async function getRecentStoredRolls(room:string,player:string,limit=100):Promise<StoredRoll[]>{const all=(await rows<RollRow>('rolls','scope',scope(room,player))).map(row=>row.roll).sort((a,b)=>a.timestamp-b.timestamp);return all.slice(-limit);}
