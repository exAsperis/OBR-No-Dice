...
type StatisticsView={selected:string;tab:AnalyticsTab|'query';includePrivate:boolean};
...
export function Statistics(){
  ...
  const [includePrivate,setIncludePrivate]=useState(restoredView?.includePrivate===true);
  ...
  const resize=()=>{
    if(!identity)return;
    try{sessionStorage.setItem(viewTransitionKey,JSON.stringify({selected,tab,includePrivate} satisfies StatisticsView));}catch{/* View state resets if storage is unavailable. */}
  ...
  const statisticsRolls=useMemo(()=>filterStatisticsRolls(rolls,includePrivate),[rolls,includePrivate]);
  return <main className={`statistics-window${maximized?' maximized':''}`}>
    <header><img className="header-icon" src="./icon.svg" alt="" aria-hidden="true"/><div><h1>Session Statistics</h1><p>{includePrivate?`All rolls visible to you · ${rolls.length} rolls`:`Public rolls only · ${statisticsRolls.length} of ${rolls.length} locally visible rolls`}</p></div><div className="statistics-header-actions"><button type="button" aria-label={maximized?'Restore statistics window':'Maximize statistics window'} title={maximized?'Restore':'Maximize'} aria-pressed={maximized} disabled={!identity} onClick={resize}><svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">{maximized?<><rect x="5" y="7" width="13" height="13" rx="1"/><path d="M8 7V4h12v12h-2"/></>:<rect x="4" y="4" width="16" height="16" rx="1"/>}</svg></button><button type="button" aria-label="Close statistics" title="Close" disabled={!identity} onClick={close}>×</button></div></header>
    <div className={`statistics-content${tab==='ledger'?' ledger-statistics-content':tab==='expressions'?' expressions-statistics-content':''}`}>
      <div className="statistics-scope-row"><label className="session-select"><span>Session</span><select value={selected} onChange={event=>setSelected(event.target.value)}>{sessions.map(item=><option key={item.id} value={item.id}>{item.endedAt?item.name:`Current Session · ${item.name}`}</option>)}</select></label><label className="statistics-audience-toggle"><input type="checkbox" checked={includePrivate} onChange={event=>setIncludePrivate(event.target.checked)}/><span className="statistics-toggle-track" aria-hidden="true"><span className="statistics-toggle-thumb"/></span><HelpTerm help="includePrivateRolls">Include private rolls</HelpTerm></label></div>
      ...
      <AnalyticsTabs tab={tab} rolls={statisticsRolls} session={selectedSession} sessionName={selectedSession?.name??'Current Session'} roomId={identity?.room??''} viewerId={identity?.player??''}/>    </div>
  </main>;
}
const rootElement=document.getElementById('root');
if(rootElement)createRoot(rootElement).render(<StrictMode><Statistics/></StrictMode>);
