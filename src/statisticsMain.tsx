...
type StatisticsView={selected:string;tab:AnalyticsTab|'query'};
...
function Statistics(){
  ...
  const resize=()=>{
    if(!identity)return;
    try{sessionStorage.setItem(viewTransitionKey,JSON.stringify({selected,tab} satisfies StatisticsView));}catch{/* View state resets if storage is unavailable. */}
