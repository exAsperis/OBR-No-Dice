...
export default function App() {
  ...
  const entry=(item:RollCardResult,isRecent=false)=><article className="entry" key={item.requestId} ref={isRecent?recentCardRef:undefined}>
    <div className="entry-meta"><strong><PlayerName roomId={obr.roomId??undefined} viewerId={obr.playerId??undefined} playerId={item.playerId} name={item.playerName}/></strong>→ {item.visibility==='everyone'?'Everyone':item.visibility==='gm'?'GM':'Self'}<span className="entry-meta-right"><span>{item.sessionName&&<> ({item.sessionName})</>} {new Date(item.time).toLocaleTimeString([], {hour:'numeric',minute:'2-digit'})}</span></span></div>
