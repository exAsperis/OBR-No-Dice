...
export function TimelineTab({rolls,roomId,viewerId}:{rolls:StoredRoll[];roomId:string;viewerId:string}){
  ...
  return <section className="timeline-tab"><h2>Timeline</h2>{!rolls.length?<p>No rolls yet.</p>:<><div className="timeline-controls"><label>Band duration (minutes)<input type="number" min="1" step="1" value={minutes} onChange={event=>setMinutes(event.target.value)}/></label></div><p className="filter-summary">{bandMinutes}-minute bands · {rolls.length} rolls<br/>Rolls run left to right in chronological order.</p><div className="timeline-legend" aria-label="Players">{players.map(player=><PlayerName key={player.id} roomId={roomId} viewerId={viewerId} playerId={player.id} name={player.name}/>)}</div><div className="timeline-size-legend"><strong>Size:</strong>{[['Low',6],['Middle',12],['High',18]].map(([label,size])=><span key={label as string}><i style={{width:size as number,height:size as number}}/>{label}</span>)}</div><p className="timeline-size-help">Disk size represents normalized result percentile. Rolls without a numeric reference distribution use the middle size.</p><div className="timeline-bands">{bands.map(band=><div className={`timeline-band${band.bandCount>1?' timeline-band-gap':''}`} key={band.start}><time className="timeline-band-time" dateTime={new Date(band.start).toISOString()} title={`${new Date(band.start).toLocaleString()} – ${new Date(band.end).toLocaleString()}`}>{band.bandCount>1?<>{time(band.start)}–{time(band.end)}<small>{band.bandCount} empty bands</small></>:time(band.start)}</time><div className="timeline-band-rolls">{band.rolls.map(roll=><TimelineRoll key={roll.id} roll={roll} position={positions.get(roll.id)??null} roomId={roomId} viewerId={viewerId}/>)}</div></div>)}</div></>}</section>;
}
