...
export class FairnessSampler {
  ...
  private total = 0;
  constructor(private expression: Node, private rng?: Rng, private overrides: readonly DieOverride[] = []) {}

  sample(amount: number): FairnessSnapshot {
    for (let i = 0; i < amount; i++) {
      const value = roll(this.expression, this.rng, false,
        {dieOverride:({node,faceCount})=>overrideFacetIndex(node,faceCount,this.overrides)}).value;
