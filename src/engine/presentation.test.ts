...
describe('whole-expression presentation', () => {
  ...
  it('presents each explosion round as a grouped reduction', () => {
    const result=roll(parse('d6!'),fixed(5,5,4));
    expect(result.stages).toEqual(['d6!','[6!]','[6 + 6!]','[6 + 6 + 5]','[17]','17']);
  ...
  it('keeps the initial exploding pool and each explosion round grouped', () => {
    const result=roll(parse('2d6!'),fixed(5,4,2));
    expect(result.stages).toEqual(['2d6!','[6!, 5]','[6 + 3, 5]','[9, 5]','14']);
    expect(result.stageDice).toEqual(['','2d6!','d6','','']);
    expect(result.stageDrawIndices).toEqual([[],[0,2],[1],[],[]]);
    const simultaneous=roll(parse('2d6!'),fixed(5,5,5,1,2));
    expect(simultaneous.stages).toEqual(['2d6!','[6!, 6!]','[6 + 6!, 6 + 3]','[6 + 6 + 2, 6 + 3]','[14, 9]','23']);
    expect(simultaneous.stageDrawIndices).toEqual([[],[0,3],[1,4],[2],[],[]]);
  });
